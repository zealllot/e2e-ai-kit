...
| Shared fixture row leaks between seed modes | A fixture row that must be **reused** across modes (because a session or saved `storageState` keys off its id) is mutable shared state. Every mode restores an unconditional baseline for every column any mode may write, *before* applying its own setup — resetting only the child rows is not enough | Run the suite in a non-default file/test order; a pass/fail that flips with order is the signature. A mode that only resets children is the smell to grep for |
